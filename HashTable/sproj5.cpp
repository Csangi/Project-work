...
//sproj5.cpp
//implementation with hiding text
#include "hashtable.h"
...
void AddUser(PassServer& PS)
{
	...
	cout << "Enter username: ";
	cin >> name;
	termios T;			//hiding text code
    tcgetattr(STDIN_FILENO, &T);
    termios hideText = T;
    hideText.c_lflag &= ~ECHO;
    tcsetattr(STDIN_FILENO, TCSANOW, &hideText);
	cout << "Enter password: ";
	cin >> password;
	tcsetattr(STDIN_FILENO, TCSANOW, &T);	//terminating the hidden text
